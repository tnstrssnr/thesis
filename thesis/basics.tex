...
\com{is this part too long to be a paragraph? but it's a bit weird to make it the only subsection}
...
\begin{definition}[Conditional Entropy for specific outcomes]
...
The table in figure \ref{tab:comp} compares the two leakage measures defined in this section. \td{finish -- clean up some of the definitions + theorems -- citations}
...
Tools like JOANA use slicing techniques on system dependence graphs to analyse the information flow in programs and give non-interference guarantees where possible \cite{hammer09}. \question{Which paper for Joana? I took the `in-depth' on from the website}.
